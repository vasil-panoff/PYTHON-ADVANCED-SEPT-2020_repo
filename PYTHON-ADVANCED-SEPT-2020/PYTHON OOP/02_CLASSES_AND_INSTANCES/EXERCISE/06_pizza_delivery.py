class PizzaDelivery:
    ...
    def pizza_ordered(self):
        self.ordered = True
        ingredient_items = ', '.join([f'{key}: {value}' for (key, value) in self.ingredients.items()])

        return f"You've ordered pizza {self.name} prepared with {ingredient_items} and the price will be {self.price}lv."
